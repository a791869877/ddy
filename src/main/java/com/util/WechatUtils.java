...
public class WechatUtils {
    ...
    @Value("${wechat.interface.config.EncodingAESKey}")
    ...
    public String getAccessToken() throws WxErrorException {
        ...
        if (RedisUtil.get("token")!=null){
            ...
            RedisUtil.set("token",accessToken,7000);
            return accessToken;
        }
    ...
    public void send(String type,String userid,String text){
        //1，配置
        ...
        if (type.equals("text")){
            message=WxMpKefuMessage.TEXT().toUser(userid).content(text).build();

        }else if (type.equals("image")){
            message=WxMpKefuMessage.IMAGE().toUser(userid).mediaId(text).build();

        }else if (type.equals("voice")){
            message=WxMpKefuMessage.VOICE().toUser(userid).mediaId(text).build();
    ...
    public WxMpMaterialNews materialNewsBatchGet(int offset, int count) throws Exception {
        ...
        try {
            ...
            if (newsBatchGetResult.getTotalCount() > 0 || newsBatchGetResult.getItemCount() > 0) {
                List<WxMpMaterialNewsBatchGetResult.WxMaterialNewsBatchGetNewsItem> items = newsBatchGetResult.getItems();
     ...
     * </pre>
     */
    public JSONObject materialCount(String materiaType) {
        ...
        try {
            ...
            jsonObject= (JSONObject) JSON.toJSON(resultCount);
        }
        catch (WxErrorException e) {
    ...
    public void clearQuota(String token) throws WxErrorException {
        wxMpService.clearQuota(token);
    }
